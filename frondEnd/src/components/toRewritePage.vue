...
export default {
        ...
        if (valid) {
          this.$confirm("确认提交吗?", "提示", {})
            .then(() => {
              ...
              api
                .updateArticle({
                  id: this.$store.state.current_article._id,
  ...
  mounted() {
    ...
    if (currentArticle) {
      this.article.classify = currentArticle.classify || "";
      this.article.title = currentArticle.title || "";
      this.article.content = currentArticle.content || "";
    }
  }
};
</script>
