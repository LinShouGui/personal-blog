<template>
    <div class="wrapper">
        <div class="main" element-loading-text="拼命加载中" v-loading="loading2">
            ...
            <article v-for='item in items' :key= "item._id">
                <header class="ArticleHeader">
                    ...
                    <div class="wrapNameClassify">
                          ...
                          <div class="ArticleName"> Created by &mdash;&raquo; 
                            <!-- <router-link :to="{path:'/selfPage'}"> -->
                                    <a @click="toSelfPage(item.name)">{{item.name}}</a>
                            <!-- </router-link> -->
                          </div>
                    </div>
                    <div>
                ...
                <section  :class="{'home_main':!isYejian,'home_main1':isYejian}"><span>{{sliceContent(item.content)}}</span></section>
                <footer>
...
export default {
  ...
  computed: {
    ...
    skeletonNode() {
      return document.querySelectorAll(".skeleton");
    },
    outsideNode() {
      return document.querySelector(".home_wrapper");
    }
  },
  methods: {
    toSelfPage(findName) {
      this.$store.dispatch("FIND_NAME", findName);
          ...
          if (code === 200) {
            setTimeout(() => {
              if (this.firstTime) {
                ...
                for (var i = 0; i < this.skeletonNode.length; i++) {
                  this.outsideNode.removeChild(this.skeletonNode[i]);
                }
              }
